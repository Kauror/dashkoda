...
`sync_ga4` and `sync_smaily` lead it and are not part of the dependency
ordering: neither shares a snapshot, an advisory lock or a source with the
feeds, so nothing waits for them and they wait for nothing.
