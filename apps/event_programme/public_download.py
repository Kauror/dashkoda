...
the event-programme feed: which setting supplies the sharing URL, which supplies
the size cap, and how the feed identifies itself in logs.

This module used to be a deliberate character-for-character copy of
``apps/legal_work/public_download.py``, with a note that a security fix to one
was a security fix owed to the other. Both now delegate to the shared module,
so a fix there reaches both feeds and the copies cannot drift.
...
WORKBOOK_SOURCE = WorkbookSource(
    ...
    log_prefix="event_programme.public_download",
)
