"""Download the canonical event-programme workbook from a public sharing link.

**This module is a deliberate copy of `apps/legal_work/public_download.py`.**
Only four things differ: the settings key, the logger name, the user-agent
string and the variable named in the not-configured error. Everything else -
the host allowlist, the loopback and IP-literal refusal, manual redirect
following, bounded retries, the streamed size cap and checksum, and the
structural XLSX proof - is character-for-character the same hardened logic.

It was copied rather than shared so that adding this feed could not change a
production one. The cost is that the two copies can drift, and a security fix
to one is a security fix owed to the other. **If you change either file, change
both**, and prefer extracting the pair into `apps/sources/` over letting them
diverge.

This module owns exactly one job: turn ``EVENT_PROGRAMME_PUBLIC_URL`` into a
validated temporary XLSX file plus a small set of non-secret facts about it. It
touches no Django model, creates no artifact, imports nothing and knows nothing
about the dashboard.

It is **not** a generic remote-file downloader. The target is one configured
workbook on a Microsoft OneDrive/SharePoint host, and there is deliberately no
way for a viewer, an administrator or a request to supply a URL.

Secret handling: the sharing URL is a bearer-style secret, because anyone
holding it can download the file. Nothing here ever logs, returns or embeds the
...
logger = logging.getLogger("dashkoda.event_programme.public_download")
...
USER_AGENT = "DashKoda/1.0 (+event-programme workbook sync)"
...
class PublicUrlNotConfigured(ImproperlyConfigured):
    """`EVENT_PROGRAMME_PUBLIC_URL` is unset or unusable."""
...
def validate_public_url(url: str) -> str:
    ...
    try:
        ...
        raise PublicUrlNotConfigured("EVENT_PROGRAMME_PUBLIC_URL ei ole kehtiv URL.") from error

    if parts.scheme.lower() != "https":
        raise PublicUrlNotConfigured("EVENT_PROGRAMME_PUBLIC_URL peab kasutama HTTPS-i.")
    ...
    try:
        ...
        raise PublicUrlNotConfigured("EVENT_PROGRAMME_PUBLIC_URL ei ole kehtiv URL.") from error
    if not hostname:
        raise PublicUrlNotConfigured("EVENT_PROGRAMME_PUBLIC_URL-il puudub hostinimi.")
    ...
    if not any(hostname == suffix or hostname.endswith(suffix) for suffix in ALLOWED_HOST_SUFFIXES):
        raise PublicUrlNotConfigured(
            "EVENT_PROGRAMME_PUBLIC_URL peab viitama Microsofti OneDrive'i või SharePointi hostile."
        )
    return hostname
...
def download_public_workbook(
    ...
    *,
    ...
    """
    source_url = url if url is not None else load_public_url()
    if url is not None:
    ...
    try:
        response = _fetch(session, build_download_url(source_url))
        try:
    ...
    logger.info(
        "event_programme.public_download completed host=%s size=%s type=%s",
        final_host,
        size,
...
def _follow(session: requests.Session, url: str) -> requests.Response:
    """One request chain: at most `MAX_REDIRECTS` hops, all HTTPS."""
    current = url
    for _hop in range(MAX_REDIRECTS + 1):
        response = _request_once(session, current)
...
def _request_once(session: requests.Session, url: str) -> requests.Response:
    try:
        return session.get(
            url,
...
def _reject_declared_size(response: requests.Response) -> None:
    limit = int(settings.EVENT_PROGRAMME_MAX_DOWNLOAD_BYTES)
...
def _stream_to_file(response: requests.Response, destination: Path) -> tuple[int, str]:
