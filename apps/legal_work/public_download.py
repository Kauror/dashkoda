...
This module owns exactly one job: turn ``OIGUSLOOME_PUBLIC_URL`` into a
...
logger = logging.getLogger("dashkoda.legal_work.public_download")
...
class PublicUrlNotConfigured(ImproperlyConfigured):
    """`OIGUSLOOME_PUBLIC_URL` is unset or unusable."""
...
def load_public_url() -> str:
    ...
    """
    url = (getattr(settings, "OIGUSLOOME_PUBLIC_URL", "") or "").strip()
    if not url:
...
def validate_public_url(url: str) -> str:
    ...
    try:
        ...
        raise PublicUrlNotConfigured("OIGUSLOOME_PUBLIC_URL ei ole kehtiv URL.") from error

    if parts.scheme.lower() != "https":
        raise PublicUrlNotConfigured("OIGUSLOOME_PUBLIC_URL peab kasutama HTTPS-i.")
    ...
    try:
        ...
        raise PublicUrlNotConfigured("OIGUSLOOME_PUBLIC_URL ei ole kehtiv URL.") from error
    if not hostname:
        raise PublicUrlNotConfigured("OIGUSLOOME_PUBLIC_URL-il puudub hostinimi.")
    ...
    if not any(hostname == suffix or hostname.endswith(suffix) for suffix in ALLOWED_HOST_SUFFIXES):
        raise PublicUrlNotConfigured(
            "OIGUSLOOME_PUBLIC_URL peab viitama Microsofti OneDrive'i või SharePointi hostile."
        )
    return hostname
...
def download_public_workbook(
    ...
    *,
    ...
    logger.info(
        "legal_work.public_download completed host=%s size=%s type=%s",
        final_host,
        size,
...
def _reject_declared_size(response: requests.Response) -> None:
    limit = int(settings.LEGAL_WORK_MAX_DOWNLOAD_BYTES)
