...
## Migrations against existing data

A fresh-database migration test and an upgrade-with-existing-data test are
different tests, and CI only ran the first kind until `legal_work` `0006`
failed in production against thirty-four rows that no test had.

A migration must come with a populated predecessor-state test when it contains
any of:

- `RunPython`, or `RunSQL` that mutates existing rows;
- a nullable → non-null transition;
- a unique constraint added to a populated table;
- a data backfill, a field derivation or a state rewrite;
- any destructive transformation.

Use the `populated_migration` fixture: it migrates to `N-1`, hands the seed the
historical models at that state, migrates to `N`, and restores the leaf
migration afterwards even if the test fails. `tests/migration_harness.py`
explains the mechanics and `tests/legal_work/test_opinion_migrations.py` is the
worked example.

Read models out of the registry the fixture gives you, never from `apps.<app>`:
today's model is not the one the migration will meet. Give a `RunPython` a
`reverse_code` — `migrations.RunPython.noop` when the forward step needs no
undoing — or the harness cannot migrate backwards to reach the predecessor
state.

Migrations that only add a table or a nullable column need no such test.
...
Warnings are a signal and the suite keeps them visible. Exactly one is filtered,
by message, in `pyproject.toml`. Do not silence a warning category — neither
globally nor with `filterwarnings` on a module — to quiet a specific noisy
warning; filter that warning's message, or fix what raises it.
