...
const CHANNELS = 'section[aria-labelledby="section-kust"]';
const MIX = 'section[aria-labelledby="section-sisujaotus"]';
const LANGUAGE = 'section[aria-labelledby="section-keeled"]';
const TOP = 'section[aria-labelledby="section-enim"]';
const RISING = 'section[aria-labelledby="section-kasvavad"]';
const OPPORTUNITIES = 'section[aria-labelledby="section-voimalused"]';
...
test("the overview answers before the reader touches anything", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  await expect(
    page.getByRole("heading", { name: "Kust tullakse", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Enim vaadatud lehed", exact: true }),
  ).toBeVisible();
});

test("the seeded history gives every headline a comparison", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  // the `pp` assertion follows the shares to the content mix, whose `Muutus`
  // column is the movement of a percentage.
  await expect(page.locator(KPI)).toContainText("%");
  await expect(page.locator(KPI)).not.toContainText("pp");
  await expect(page.locator(MIX)).toContainText("pp");
});

test("a period the history cannot fill is offered and inert", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("the chart bundle loads only where there is something to draw", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("no language root or utility path occupies a ranking position", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  /*
   * `Enim vaadatud lehed` is a table since 2026-08-18, so its rows are read
   * from the section itself rather than from a chart payload.
   */
  ...
  for (const excluded of [
    ...
    expect(labels, `${excluded} must not be ranked as content`).not.toContain(
      excluded,
    );
  }
...
test("the excluded traffic still counts towards the site's own figures", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("the content mix no longer states its denominator inline", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  await expect(
    page.getByRole("heading", { name: "Vaadatud sisu jaotus" }),
  ).toBeVisible();
  await expect(page.locator("main")).not.toContainText("järjestatavaks sisuks");
});

test("the movement list finds the pages the seed made move", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  const rising = page.locator(RISING);
  await expect(rising).toContainText("Kasvavad lehed");
  await expect(rising).not.toContainText("Halvimad");
  await expect(rising).toContainText("sunteetiline-kasvav");

  // `Vähenenud tähelepanu`, the declining half, left the page on 2026-08-17
  // and its builder on 2026-08-18. `sunteetiline-vaibuv` was the seed's
  // declining fixture; it rendered only there and is gone with it.
  await expect(page.locator("main")).not.toContainText("Vähenenud tähelepanu");
  await expect(rising).not.toContainText("sunteetiline-vaibuv");
});
...
test("a page with no previous measurement states that rather than a percentage", async ({
  ...
  await expect(page.locator(RISING)).toContainText("uus mõõdetud liiklus");
});

test("the language split disclaims what it does not measure", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  // A stacked bar and its legend since 2026-08-18, so the languages are read
  // from the legend rather than from a chart payload.
  const language = page.locator(LANGUAGE);
  await expect(language).toContainText("Eesti");
  await expect(language).toContainText("Inglise");
  await expect(language).toContainText("mitte külastaja rahvust");
});

test("the opportunity tiles name measurements rather than verdicts", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  /*
   * The engagement matrix was drawn until 2026-08-18 and is now read only by
   * the three tiles beneath the rankings. Each tile states its rule's own name
   * and the measurements that triggered it — evidence, never a verdict, which
   * is the property this test exists to hold.
   */
  const tiles = page.locator(OPPORTUNITIES);
  await expect(tiles.locator("li").first()).toBeVisible();
...
test("no composite health score is invented", async ({ page }) => {
  ...
  for (const focus of ["ulevaade", "liiklus", "sisu", "kanalid", "lehed"]) {
    ...
    expect(text).not.toMatch(
      /Health Score|Digital Score|Engagement Score|\d+\s*\/\s*100/,
    );
  }
});
...
test("channels are shown with the denominator their shares used", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  await expect(
    page.getByRole("heading", { name: "Kust tullakse" }),
  ).toBeVisible();
  // A table with a bar on its leading figure since 2026-08-18, so the channels
  // are read from the rows rather than from a chart payload.
  await expect(page.locator(CHANNELS)).toContainText("Organic Search");
});
...
test("searching reaches a page the ranking does not show", async ({ page }) => {
  ...
  await expect(page.locator(TOP)).not.toContainText(wanted);
  ...
  await page
    .locator(SEARCH)
    .getByRole("button", { name: "Otsi", exact: true })
    .click();

  await expect(page.locator(SEARCH)).toContainText(wanted);
  // And the box still holds what was typed, so the term stays visible.
  await expect(page.getByLabel("Lehe nimi või aadress")).toHaveValue(
    "pealkiri 12",
  );
});

test("a search that finds nothing keeps its box and offers the way back", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("the period travels with a search and survives clearing it", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  await expect(active()).toHaveText(["90 päeva"]);
  await expect(page.locator(SEARCH)).not.toContainText(
    "Ühtegi lehte ei leitud",
  );
});

test("results paginate and each page keeps the whole query", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("one page opens its own analysis with both of its figures", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
  ...
  await page
    .locator("#koduleht-otsingutulemused")
    .getByRole("link")
    .first()
    .click();
...
test("the coverage disclosure moved to Admin and kept its numbers", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("Koduleht no longer carries the disclosure it used to end with", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
...
test("the seeded gaps are disclosed rather than smoothed over", async ({
  page,
}) => {
  oncePerRun();
  await signIn(page);
   ...
   */
  await page.goto("/koduleht/?fookus=sisu&periood=30");
  await expect(page.locator(RISING)).toContainText("Kasvavad lehed");
  ...
  const counts = await method.evaluate((node) => {
    ...
    return {
      days: read("Kogutud päevi"),
      pages: read("Lehekaupa kogutud päevi"),
    };
  });
